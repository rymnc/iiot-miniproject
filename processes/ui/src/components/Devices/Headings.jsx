    ...
    return (
        headings.map((header, i) => (
            <th className="text-center" key={i}>
                {header}
                {isHighlighted(header, 'asc') ? < CaretUpFilled {...deactivate()} /> : <CaretUp {...activate(header, 'asc')} />}
                {isHighlighted(header, 'desc') ? < CaretDownFilled {...deactivate()} /> : <CaretDown {...activate(header, 'desc')} />}
            </th>
        ))
    )
}
